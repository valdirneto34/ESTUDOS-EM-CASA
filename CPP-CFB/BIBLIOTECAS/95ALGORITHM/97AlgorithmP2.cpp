...
int main()
{
    ...
    // equat = Verifica se os elementos de uma coleção são iguais aos de outra
    cout << "EQUAL" << endl;
    ...
    if (it != vt1.end())
        ...
        cout << "Colecao não encontrada" << endl;
    cout << "\n";
    ...
    cout << "COPY_IF" << endl;
    copy_if(vt5.begin(), vt5.end(), vt1.begin(), [](int i)
            { return (i % 2) == 0; }); // Copia somente os pares
    for (auto x : vt1)
        cout << x << "  ";
    cout << "\n\n";

    // move = Move elementos de uma coleção para outa
    cout << "MOVE" << endl;
    vt4 = move(vt1);
    for (auto x : vt4)
        cout << x << "  ";
    ...
    // swap = Ytoca os elementos de duas funções
    swap(vt2, vt3);

    // transform = Aplica uma função que altera os valores de uma coleção
    cout << "TRANSFORM" << endl;
    transform(vt5.begin(), vt5.end(), vt5.begin(), [](int i)
              { return i * 100; });
    for (auto x : vt5)
        cout << x << "  ";
    cout << "\n\n";

    // replace = Substitui um determinado valor de uma coleção por outro
